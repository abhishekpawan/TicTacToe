...
// Dedicated socket.io endpoint for Vercel
app.post('/api/socket', (req, res) => {
  res.json({ 
    message: 'Socket.io is not available in serverless mode. Please use the REST endpoints.',
    ...
    socket.on('make-move', async ({ roomId, position }) => {
      ...
      if (winner || isDraw) {
        ...
        const processPlayerStats = async (playerSocket: any, playerMark: 'X' | 'O') => {
          ...
          // Skip if player not authenticated - but this is optional for gameplay
